...
class FlightUIEnhanced:
    ...
    def _create_all_ui_elements(self):
        ...
        # Left corrections
        self.buttons['left_10'] = self._create_button('←← 10°', 50, button_y, 80, 35, self._correct_left_10)
        self.buttons['left_5'] = self._create_button('← 5°', 140, button_y, 70, 35, self._correct_left_5)
        self.buttons['left_1'] = self._create_button('← 1°', 220, button_y, 60, 35, self._correct_left_1)

        # Center controls
        self.buttons['autopilot'] = self._create_button('AUTO', 320, button_y, 80, 35, self._toggle_autopilot)
        self.buttons['center'] = self._create_button('CENTER', 410, button_y, 80, 35, self._center_heading)

        # Right corrections
        self.buttons['right_1'] = self._create_button('1° →', 520, button_y, 60, 35, self._correct_right_1)
        self.buttons['right_5'] = self._create_button('5° →', 590, button_y, 70, 35, self._correct_right_5)
        self.buttons['right_10'] = self._create_button('10° →→', 670, button_y, 80, 35, self._correct_right_10)

        # System controls
        self.buttons['legend'] = self._create_button('LEGEND', 800, button_y, 80, 35, self._toggle_legend)
        self.buttons['help'] = self._create_button('HELP', 890, button_y, 60, 35, self._toggle_help)
        self.buttons['end_flight'] = self._create_button('END FLIGHT', self.width - 130, button_y, 110, 35, None)

        # Speed controls
        speed_y = button_y + 45
        self.buttons['speed_down'] = self._create_button('SPEED -', 320, speed_y, 80, 30, None)
        self.buttons['speed_up'] = self._create_button('SPEED +', 410, speed_y, 80, 30, None)
    ...
    def _update_animations(self, dt):
        ...
        # Flashing elements
        self.animations['warning_flash'] = (self.animations['warning_flash'] + dt * 5) % 2
        ...
        if status['is_flying']:
            ...
            if phase == 'climb':
                self.animations['horizon_pitch'] = min(15, self.animations['horizon_pitch'] + dt * 5)
            elif phase == 'descent':
                self.animations['horizon_pitch'] = max(-15, self.animations['horizon_pitch'] - dt * 5)
            else:
    ...
    def _draw_no_flight_screen(self):
        ...
        # Title
        title = self.fonts['huge'].render("FLIGHT SIMULATOR", True, self.colors['text_highlight'])
        title_rect = title.get_rect(center=(self.width // 2, self.height // 2 - 60))
        self.screen.blit(title, title_rect)

        # Subtitle with animation
        alpha = int(128 + 127 * math.sin(self.frame_count * 0.05))
        subtitle = self.fonts['medium'].render("No Active Flight", True, (*self.colors['text_secondary'], alpha))
        subtitle_rect = subtitle.get_rect(center=(self.width // 2, self.height // 2))
        self.screen.blit(subtitle, subtitle_rect)

        # Instruction
        instruction = self.fonts['normal'].render("Select 'Travel to Airport' from the main menu to start a flight", True, self.colors['text_dim'])
    ...
    def _draw_status_bar(self, status):
        ...
        pygame.draw.rect(self.screen, self.colors['panel_bg'], self.panels['status_bar'])
        pygame.draw.rect(self.screen, self.colors['panel_border'], self.panels['status_bar'], 2)
        ...
        phase_color = phase_colors.get(status['flight_phase'].upper(), self.colors['text_primary'])
        ...
        time_text = self.fonts['medium'].render(f"TIME: {time_str}", True, self.colors['text_primary'])
    ...
    def _draw_primary_flight_display(self, status):
        ...
        self._draw_vertical_tape(
            panel.right - 80, center_y - 60, 60, 200,
            status['altitude'], 0, 50000, 'ALT', 'ft',
            self.colors['gauge_green']
        )

        # === SPEED TAPE ===
        self._draw_vertical_tape(
            panel.left + 20, center_y - 60, 60, 200,
            status['airspeed'], 0, 600, 'IAS', 'kts',
            self.colors['gauge_green']
        )

        # === COMPASS ===
        self._draw_compass(center_x, panel.bottom - 60, 50, status['heading'], status['target_heading'])
    ...
    def _draw_attitude_indicator(self, x, y, size, status):
        ...
        if abs(roll) > 0.1:
            ...
            pygame.draw.arc(self.screen, self.colors['text_primary'], arc_rect,
                          math.radians(270 - 30), math.radians(270 + 30), 2)

            # Roll indicator mark
            roll_angle = math.radians(270 + roll)
            roll_x = x + (size - 15) * math.cos(roll_angle)
            roll_y = y + (size - 15) * math.sin(roll_angle)
            pygame.draw.circle(self.screen, self.colors['warning'], (int(roll_x), int(roll_y)), 5)

        # Remove clipping
        self.screen.set_clip(None)

        # Aircraft symbol (fixed)
        pygame.draw.line(self.screen, self.colors['text_highlight'], (x - 30, y), (x + 30, y), 3)
        pygame.draw.line(self.screen, self.colors['text_highlight'], (x, y - 5), (x, y + 5), 3)
        ...
        for i in range(-2, 3):
            if i != 0:
                ...
                pygame.draw.line(self.screen, self.colors['text_primary'],
                               (x - width, mark_y), (x + width, mark_y), 1)
                if i % 2 == 0:
                    pitch_text = self.fonts['tiny'].render(str(abs(i * 10)), True, self.colors['text_dim'])
                    self.screen.blit(pitch_text, (x + width + 5, mark_y - 5))

    def _draw_vertical_tape(self, x, y, width, height, value, min_val, max_val, label, unit, color):
        ...
        for i in range(-10, 11):
            ...
            if min_val <= mark_value <= max_val:
                ...
                if mark_y > y and mark_y < y + height:
                    ...
                    if mark_value % 50 == 0:
                        pygame.draw.line(self.screen, self.colors['text_primary'],
                                       (x + width - 15, mark_y), (x + width, mark_y), 2)
                        if mark_value % 100 == 0:
                            mark_text = self.fonts['tiny'].render(str(int(mark_value)), True, self.colors['text_dim'])
                            self.screen.blit(mark_text, (x + 5, mark_y - 6))
                    else:
                        pygame.draw.line(self.screen, self.colors['text_dim'],
                                       (x + width - 8, mark_y), (x + width, mark_y), 1)

        # Current value text
        value_text = self.fonts['normal'].render(f"{int(value)}", True, self.colors['text_highlight'])
        value_text_rect = value_text.get_rect(center=(x + width // 2, y + height // 2))
        self.screen.blit(value_text, value_text_rect)

        # Label
        label_text = self.fonts['small'].render(label, True, self.colors['text_primary'])
    ...
    def _draw_compass(self, x, y, size, heading, target_heading):
        ...
        for angle in range(0, 360, 10):
            ...
            pygame.draw.line(self.screen, self.colors['text_primary'], (x1, y1), (x2, y2), mark_width)
            ...
            if angle % 90 == 0:
                ...
                if angle in directions:
                    ...
                    dir_text = self.fonts['small'].render(directions[angle], True, self.colors['text_highlight'])
        ...
        pygame.draw.circle(self.screen, self.colors['info'], (int(target_x), int(target_y)), 6)

        # Aircraft indicator (center)
        pygame.draw.polygon(self.screen, self.colors['warning'],
                           [(x, y - 10), (x - 5, y + 5), (x + 5, y + 5)])

        # Heading text
        heading_text = self.fonts['normal'].render(f"{int(heading)}°", True, self.colors['text_highlight'])
        heading_rect = heading_text.get_rect(center=(x, y + size + 20))
        self.screen.blit(heading_text, heading_rect)

        # Target heading text
        target_text = self.fonts['small'].render(f"Target: {int(target_heading)}°", True, self.colors['info'])
    ...
    def _draw_vsi(self, x, y, size, status):
        ...
        pygame.draw.line(self.screen, self.colors['gauge_needle'], (x, y), (needle_x, needle_y), 2)
    ...
    def _draw_navigation_display(self, status):
        ...
        # Map area
        map_rect = pygame.Rect(panel.x + 20, panel.y + 40, panel.width - 40, panel.height - 80)
        ...
        for i in range(1, 5):
            ...
            pygame.draw.line(self.screen, self.colors['map_grid'],
                           (x_line, map_rect.y), (x_line, map_rect.bottom), 1)
            pygame.draw.line(self.screen, self.colors['map_grid'],
                           (map_rect.x, y_line), (map_rect.right, y_line), 1)
        ...
        if hasattr(flight_simulator, 'current_flight') and flight_simulator.current_flight:
            ...
            pygame.draw.line(self.screen, self.colors['map_route'], (start_x, start_y), (end_x, end_y), 3)

            # Departure point
            pygame.draw.circle(self.screen, self.colors['normal'], (start_x, start_y), 8)
            dep_text = self.fonts['tiny'].render("DEP", True, self.colors['text_primary'])
            self.screen.blit(dep_text, (start_x - 15, start_y + 10))

            # Destination point
            pygame.draw.circle(self.screen, self.colors['warning'], (end_x, end_y), 8)
            dest_text = self.fonts['tiny'].render("DEST", True, self.colors['text_primary'])
            ...
            if flight_simulator.current_flight:
                distance_remaining = flight_simulator.current_flight.distance_nm * (1 - progress)
                dist_text = self.fonts['small'].render(f"{distance_remaining:.1f} nm to go", True, self.colors['text_primary'])
                self.screen.blit(dist_text, (map_rect.centerx - 50, map_rect.bottom - 30))
        ...
        pygame.draw.line(self.screen, (0, 255, 0, 50), (panel.centerx, panel.centery), (sweep_x, sweep_y), 2)
    ...
    def _draw_aircraft_icon(self, x, y, heading):
        ...
        # Left wing
        points.append((x + size * 0.7 * math.cos(angle + 2.5), y + size * 0.7 * math.sin(angle + 2.5)))
        # Tail
        points.append((x - size * 0.5 * math.cos(angle), y - size * 0.5 * math.sin(angle)))
        # Right wing
        points.append((x + size * 0.7 * math.cos(angle - 2.5), y + size * 0.7 * math.sin(angle - 2.5)))
    ...
    def _draw_engine_display(self, status):
        ...
        self._draw_gauge_bar(panel.x + 20, y_offset, panel.width - 40, 30,
                            status['engine_temp'], 0, 250, "TEMP", "°F",
        ...
        # Fuel gauge
        self._draw_gauge_bar(panel.x + 20, y_offset, panel.width - 40, 30,
                            status['fuel_remaining'], 0, 100, "FUEL", "%",
        ...
        self._draw_digital_display(panel.x + 20, y_offset, panel.width - 40, 40,
                                  f"{voltage:.1f}V", "ELEC", self.colors['normal'])
        ...
        self._draw_digital_display(panel.x + 20, y_offset, panel.width - 40, 40,
                                  f"{int(hyd_pressure)}", "HYD", self.colors['normal'])

    def _draw_gauge_bar(self, x, y, width, height, value, min_val, max_val, label, unit, color_zones):
        """Draw a horizontal bar gauge with color zones."""
        # Background
        pygame.draw.rect(self.screen, self.colors['gauge_bg'], (x, y, width, height))
        pygame.draw.rect(self.screen, self.colors['panel_border'], (x, y, width, height), 1)
        ...
        for zone_min, zone_max, color in color_zones:
            if min_val <= zone_max and max_val >= zone_min:
                ...
                zone_width = (min(zone_max, max_val) - max(zone_min, min_val)) / (max_val - min_val) * width
                pygame.draw.rect(self.screen, (*color, 50), (zone_x, y, zone_width, height))
        ...
        value_text = self.fonts['small'].render(f"{value:.1f}{unit}", True, self.colors['text_primary'])
    ...
    def _draw_radio_panel(self, status):
        ...
        pygame.draw.line(self.screen, self.colors['panel_border'],
                        (panel.x + 20, y_offset), (panel.right - 20, y_offset), 1)
        y_offset += 10

        xpdr_text = self.fonts['normal'].render("XPDR: 1200", True, self.colors['normal'])
    ...
    def _draw_flight_info_panel(self, status):
        ...
        if hasattr(flight_simulator, 'current_flight') and flight_simulator.current_flight:
            ...
            info_items = [
                ...
                ("DIST", f"{flight_plan.distance_nm:.0f} nm", self.colors['text_secondary']),
                ("ETA", self._calculate_eta(status), self.colors['info']),
                ("FL", f"{flight_plan.cruise_altitude // 100:03d}", self.colors['text_secondary']),
                ("TAS", f"{flight_plan.cruise_speed} kts", self.colors['text_secondary']),
            ]

            for label, value, color in info_items:
                label_surface = self.fonts['small'].render(f"{label}:", True, self.colors['text_dim'])
    ...
    def _draw_weather_panel(self, status):
        ...
        condition_color = condition_colors.get(condition_text, self.colors['text_primary'])

        cond_surface = self.fonts['medium'].render(condition_text, True, condition_color)
        self.screen.blit(cond_surface, (panel.x + 20, y_offset))
        y_offset += 35

        # Wind information
        wind_text = f"WIND: {weather['wind_direction']:03d}° @ {weather['wind_speed']} kts"
        wind_surface = self.fonts['small'].render(wind_text, True, self.colors['text_primary'])
        self.screen.blit(wind_surface, (panel.x + 20, y_offset))
        y_offset += 25

        # Visibility
        vis_text = f"VIS: {weather['visibility']} miles"
        vis_surface = self.fonts['small'].render(vis_text, True, self.colors['text_primary'])
        ...
        press_surface = self.fonts['small'].render(press_text, True, self.colors['text_secondary'])
    ...
    def _draw_alerts_panel(self, status):
        ...
        if status['system_alerts']:
            ...
            # No alerts
            no_alerts = self.fonts['normal'].render("No active alerts", True, self.colors['normal'])
    ...
    def _draw_control_panel(self, status):
        ...
        for y in range(panel.height):
            ...
            pygame.draw.line(self.screen, (color_value, color_value, color_value + 10),
                           (panel.x, panel.y + y), (panel.right, panel.y + y))
        ...
        heading_label = self.fonts['normal'].render("HEADING CONTROL", True, self.colors['text_primary'])
        ...
        diff_color = self.colors['normal'] if abs(heading_diff) < 5 else self.colors['warning']
        ...
        hints_surface = self.fonts['tiny'].render(hints_text, True, self.colors['text_dim'])
    ...
    def _draw_panel_background(self, rect, title, border_color=None):
        ...
        # Title text
        title_surface = self.fonts['small'].render(title, True, self.colors['text_primary'])
    ...
    def _draw_progress_bar(self, x, y, width, height, percent):
        ...
        # Progress text
        progress_text = self.fonts['tiny'].render(f"{percent:.1f}%", True, self.colors['text_primary'])
    ...
    def _draw_warning_overlays(self, status):
        ...
        if status['emergency_state']:
            ...
            if self.animations['warning_flash'] > 1:
                ...
                # Emergency text
                emergency_text = self.fonts['huge'].render("EMERGENCY", True, self.colors['danger'])
            ...
            if self.animations['warning_flash'] > 1:
                warning_text = self.fonts['large'].render("COURSE DEVIATION", True, self.colors['warning'])
    ...
    def _draw_legend_overlay(self):
        ...
        # Title
        title = self.fonts['large'].render("INSTRUMENT LEGEND", True, self.colors['text_highlight'])
        ...
        legend_items = [
            ...
            ("  IAS - Indicated Airspeed in knots", self.colors['text_secondary'], False),
            ("  HDG - Heading in degrees", self.colors['text_secondary'], False),
            ("  Attitude Indicator - Shows aircraft pitch and roll", self.colors['text_secondary'], False),
            ...
            ("  TEMP - Engine temperature (keep below 220°F)", self.colors['text_secondary'], False),
            ("  FUEL - Fuel remaining percentage", self.colors['text_secondary'], False),
            ("  OIL - Oil pressure in PSI", self.colors['text_secondary'], False),
            ("  ELEC - Electrical system voltage", self.colors['text_secondary'], False),
        ...
        # Close instruction
        close_text = self.fonts['normal'].render("Press 'L' or click LEGEND to close", True, self.colors['text_dim'])
        close_rect = close_text.get_rect(centerx=legend_rect.centerx, bottom=legend_rect.bottom - 20)
    ...
    def _draw_help_overlay(self):
        ...
        # Title
        title = self.fonts['large'].render("FLIGHT CONTROLS HELP", True, self.colors['text_highlight'])
        ...
        help_items = [
            ...
            ("Shift + ← / → : Turn left/right 5 degrees", self.colors['text_primary'], False),
            ...
            ("• Watch your heading - aircraft constantly drifts", self.colors['text_secondary'], False),
            ("• Keep heading close to target heading (blue indicator)", self.colors['text_secondary'], False),
            ("• Monitor engine temperature - don't let it overheat", self.colors['text_secondary'], False),
            ("• Watch fuel gauge - emergency landing if you run out", self.colors['text_secondary'], False),
            ("• Autopilot helps but doesn't eliminate all drift", self.colors['text_secondary'], False),
            ("• Weather affects how much the aircraft drifts", self.colors['text_secondary'], False),
            ("", self.colors['text_primary'], False),
            ("DESERT BUS MODE", self.colors['warning'], True),
            ("This flight simulation requires constant attention!", self.colors['text_secondary'], False),
            ("The aircraft will drift off course if you don't correct it.", self.colors['text_secondary'], False),
            ("Real-time flights can take hours - stay alert!", self.colors['text_secondary'], False),
        ]
        ...
        # Close instruction
        close_text = self.fonts['normal'].render("Press 'H' or click HELP to close", True, self.colors['text_dim'])
        close_rect = close_text.get_rect(centerx=help_rect.centerx, bottom=help_rect.bottom - 20)
        self.screen.blit(close_text, close_rect)
    ...
    def _calculate_eta(self, status):
        ...
        if not hasattr(flight_simulator, 'current_flight') or not flight_simulator.current_flight:
            return "N/A"
